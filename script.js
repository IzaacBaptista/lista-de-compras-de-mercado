...
// update function
const updateItem =
    function(currentItemIndex, value, price, qtd) {
...
const handleItem = function(itemData) {
  ...
  items.forEach((item) => {
    // done
    if (item.querySelector(".title").getAttribute('data-time') ==
        itemData.addedAt) {
      item.querySelector('[data-done]').addEventListener('click', function(e) {
        ...
        const currentClass =
            currentItem.isDone ? "bi-check-circle-fill" : "bi-check-circle";
...
const getList =
  ...
  if (todoItems.length > 0) {
    todoItems.forEach((item) => {
      const iconClass =
          item.isDone ? "bi-check-circle-fill" : "bi-check-circle";

      let liTag = `
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <span class="fs-5 title" data-time=${item.addedAt}>${
          item.name}</span>
...
const setLocalStorage =
  ...
  localStorage.setItem("todoItems", JSON.stringify(todoItems));
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      // filter tabs
      filters.forEach((tab) => {tab.addEventListener('click', function(e) {
                        e.preventDefault();
